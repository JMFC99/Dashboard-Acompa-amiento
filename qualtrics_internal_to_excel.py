...
BASIC_ELEMENTS = {
    ...
    'País de Residencia:':'País Residencia',
...
def process_coaches_data(input_file, output_file, verbose=False):
    ...
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        # Save basic info
        coaches_internos.to_excel(writer, sheet_name='Basic Info', index=False)
    ...
    print(f"✓ Total sheets created: {len(all_dataframes) + 1}")
